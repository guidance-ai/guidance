...
def get_model(model_name, caching=False, **kwargs):
    ...
    elif model_name.startswith("azure_guidance_chat:"):
        return get_azure_guidance_chat_model(model_name[20:], caching, **kwargs)
...
azure_guidance_chat_model_cache = {}
azure_guidance_chat_defaults = {}


def get_azure_guidance_chat_model(model_name, caching=False, **kwargs):
    """Get Azure Guidance LLM with model reuse."""
    ...
    kwargs = kwargs.copy()
    for key, val in azure_guidance_chat_defaults.items():
        if key not in kwargs:
            kwargs[key] = val
    ...
    # load it over and over again
    key = model_name + "_" + str(caching) + "_" + str(kwargs)
    if key not in azure_guidance_chat_model_cache:
        azure_guidance_chat_model_cache[key] = guidance.models.AzureGuidanceChat(
            model_name, **kwargs
        )

    return azure_guidance_chat_model_cache[key]
