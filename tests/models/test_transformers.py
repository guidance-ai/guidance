...
def test_phi3_loading():
    ...
    lm = models.Transformers(r"microsoft/Phi-3-mini-4k-instruct", trust_remote_code=True)
    lm += f"""Finish counting to 5: 1,2,3,4, + {gen("five", max_tokens=1)}"""
    assert lm['five'] == '5'

def test_phi3_chat():
    from guidance import models, system, user, assistant
    lm = models.TransformersChat(r"microsoft/Phi-3-mini-4k-instruct", trust_remote_code=True)
    with system():
