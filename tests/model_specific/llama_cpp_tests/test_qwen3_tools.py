...
import guidance
from guidance import assistant, gen, special_token, system, user
...
{"name": <function-name>, "arguments": <args-json-object>}
    ...
    with assistant():
        lm += special_token("<think>") + gen(max_tokens=100) + special_token("</think>") + gen(tools=[trending_songs])

    with assistant():
        lm += special_token("<think>") + gen(max_tokens=100) + special_token("</think>") + gen(name="follow_up")
