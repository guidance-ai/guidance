            ...
            while True: # each iteration generates one more token (and some of the associated bytes)
                ...
                if logits is not None:
                    ...
                    if not is_forced and token_pos < len(sampled_token) and trie == self._token_trie:
                        ...
                        is_response = True
                        response_new_bytes = new_bytes[hidden_count:]
                        response_is_generated = is_generated
                        response_new_bytes_prob = new_bytes_prob if self.compute_log_probs else 1.0
                        response_capture_groups = captured_data
                        response_capture_group_log_probs = captured_log_prob_data
                        response_new_token_count = token_count - last_token_count
