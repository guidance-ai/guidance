...
async def assistant(block_content, partial_output, parser, prev_node, next_node, next_next_node, hidden=False):
    ...
    '''
    return await role(role="assistant", block_content=block_content, partial_output=partial_output, parser=parser, prev_node=prev_node, next_node=next_node, next_next_node=next_next_node, hidden=hidden)
