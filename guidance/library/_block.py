...
def block(lm, name=None, open_text="", close_text="", hidden=False):
    ...
    def __exit__(lm, exc_type, exc_value, traceback):
        _rec_close(lm, open_text, close_text, text_name=name, text_offset=offset, hidden=hidden)
...
def _rec_close(lm, open_text, close_text, text_name=None, text_offset=0, hidden=False):
    if text_name is not None:
        lm[text_name] = TextRange(text_offset, len(lm), lm)
    if close_text != "":
        lm += close_text
    if hidden:
        lm.reset(text_offset - len(open_text))
