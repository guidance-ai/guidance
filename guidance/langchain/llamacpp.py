...
class LlamaCpp_LLM(LLM):
    ...
    """The LlamaCpp_LLM class allow to to use an llm loaded with Guidance as a Langchain llm object.
    
    Example:
        import guidance
        from guidance.langchain import LlamaCpp_LLM

        llama2 = guidance.models.MistralChat(model=modelPath)
        llm = LlamaCpp_LLM(model=llama2)

        prompt = "Question: A rap battle between Stephen Colbert and John Oliver"
    ...
    model: Any #: :guidance llamacpp object
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        try:
            for key, val in values["client"].__dict__.items():
