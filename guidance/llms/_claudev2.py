...
# transforming the parsed prompt into a list of messages, where each message is a dictionary containing a 'role', 'content', and potentially additional properties.
...
# claude uses completion prop instead of choices
        ...
        if "completion" in resp:
            for c in resp['completion']:
                ...
                if not found_content and not in_function_call:
                    break # the role markers are outside the generation in chat mode right now
            else:
                yield resp
        else:
            yield resp
...
class Claude(LLM):
    ...
    def __init__(self, model=None, caching=True, max_retries=5, max_calls_per_min=60,
        ...
        if api_base is None:
            api_base = os.environ.get("ANTHROPIC_API_BASE", None) or os.environ.get("ANTHROPIC_ENDPOINT", None)
