...
class Transformers(LLM):
    ...
    def session(self, asynchronous=False):
        sess = TransformersSession
        if self.tgi:
            sess = TGITransformersSession

        if asynchronous:
            return sess(self)
        else:
            return SyncSession(sess(self))
...
class TGITransformersSession(TransformersSession):
    def __enter__(self):
        return self
        ...
        **generate_kwargs,
        ...
        if stop is not None:
            if isinstance(stop, str):
                ...
                stop_regex = [s for s in stop]
        if isinstance(stop_regex, str):
            stop_regex = [stop_regex]
        if stop_regex is None:
            stop_regex = []
        stop_regex.append(
            self.llm.tokenizer.eos_token
        )  # make sure the end of sequence token is always included
        ...
        if True:
            ...
            # a streamer to handle potentially partial output
            streamer = TGITransformersStreamer()

            generate_args = {
                "stop_sequences": stop_regex,
            ...
            generated_res = await self.llm.model_obj.generate(prompt, **generate_args)
            logger.debug(f"output text:  {generated_res['generated_text']}")

            streamer.put(generated_res)
            res = streamer.__next__()
            self._update_prefix_cache(streamer)
        return res


class TGITransformersStreamer(TransformersStreamer):
    FINISH_REASON_MAP = {
        "stop_sequence": "stop",
        "eos_token": "endoftext",
        "length": "length",
