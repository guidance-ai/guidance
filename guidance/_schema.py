...
class GenToken(BaseModel):
    ...
    bytes: bytes
    latency_ms: float
    is_generated: bool  # true if the token was generated by the engine model, false if it was forced forwards by the parser


class EngineOutput(BaseModel):
    # token that was issued by the engine in one iteration
...
class VisBytesChunk(BaseModel):
    bytes: bytes
    is_input: bool = False
    is_generated: bool  # true if coming from engine/parser, false if it's user input
    engine_outputs: list[EngineOutput] = []  # list of associated tokens generated by the engine
    backtrack: NonNegativeInt = (
        0  # number of tokens was backtracked by the parser before this chunk
    )
