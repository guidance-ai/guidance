...
<div class="w-full">
	...
	<nav class="sticky top-0 z-50 opacity-90 w-full flex bg-gray-100 text-gray-500 justify-between">
		<div class="pl-2 flex">
			...
			<div class={`${i === 2 ? "bg-gray-600" : ""} flex flex-col items-center py-1 px-4 hover:bg-gray-600 hover:cursor-pointer group`}>
				<div class={`uppercase tracking-wider text-xs ${i === 2 ? "text-gray-300" : "text-gray-500"} group-hover:text-gray-300`}>{metric.name}</div>
				{#if metric.value.constructor === Array}
				<Minibar values={metric.value} svgClass={"w-12 h-6"} rectClass={`${i === 2 ? "fill-gray-100" : "fill-gray-700"} group-hover:fill-gray-100`} padding={padding}/>
				{:else}
				<div class={`text-lg ${i === 2 ? "text-gray-100" : "text-gray-700"} group-hover:text-gray-100`}>{metric.value}<span class="text-xs pl-1">{metric.units}</span></div>
				{/if}
			</div>
			{/each}
		</div>
