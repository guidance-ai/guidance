<script lang="ts">
    ...
    import { type TextOutput } from './stitch';

    export let tokens: ArrayLike<TextOutput> = [];
    ...
    <div class="px-4">
        <span class="flex flex-wrap text-sm">
            {#each tokens as token, i}
                {#if token.value.match(imagePattern)}
                    <span class="pb-1 inline-block mt-2 border-b-2 hover:bg-gray-300 hover:brightness-75" style={`border-bottom-color: ${color(token.prob)}`}>
                        <img src={`data:image/jpeg;base64,${token.value.match(imagePattern)?.[1] || ""}`} alt="inlined img"/>
                    </span>
