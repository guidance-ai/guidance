<script lang="ts">
  ...
  function updateProgress() {
    if (audio) {
      ...
      // Redraw waveform to update the colored portion
      drawWaveform();
    }
  }
  ...
  function seek(event: MouseEvent) {
    ...
    const seekPosition = (event.offsetX / container.offsetWidth) * audio.duration;
  ...
  async function drawWaveform() {
    ...
    // Only compute waveform data once
    ...
    // Calculate the cutoff point based on current progress
    const progressPixel = Math.floor((progress / 100) * width);
    
    // Draw the waveform bars with SoundCloud style
    const barWidth = 2;
    const gap = 1;
    const totalBars = Math.floor(width / (barWidth + gap));
    
    for (let i = 0; i < totalBars; i++) {
      // Calculate appropriate amplitude for this position
      const dataIndex = Math.floor((i / totalBars) * waveformData.length);
      const normalizedAmp = waveformData[dataIndex] / maxAmp;
      
      const barHeight = normalizedAmp * height * 0.8; // 80% of height for aesthetics
    ...
    // Draw progress indicator line (optional)
    if (progress > 0) {
      ctx.beginPath();
    ...
    // Draw hover indicator if mouse is over waveform (optional)
    if (hoverPosition >= 0) {
      ...
      <div class="relative"
        ...
        >
          <svg class="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
            ...
            {:else}
              <!-- High volume icon -->
              <path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.07c1.48-.74 2.5-2.26 2.5-4.04z"></path>
            {/if}
          </svg>
        </button>
        ...
        {#if showVolumeSlider}
          ...
          >
            <div class="w-24 relative h-2 rounded-full bg-gray-200">
              <input 
                type="range" 
                min="0" 
                max="1" 
