...
#GroupbyAgg
group_by_agg_str ="""{{~#if this.command=="GroupbyAgg"}}
            "command_args":{
...
group_by_transform_str = """
            {{~#if this.command=="GroupbyTransform"}}
            "command_args":{
                "by": [{{~gen "this.by" temperature=0.01 stop=']'}}],
                "transform_args": {
                    "transfrom_cols": [{{~gen "this.transfrom_cols" temperature=0.01 stop=']'}}],
                    "function_name": "{{~gen "this.function_name" temperature=0.01 stop='"'}}"
                    },
...
feature_trans_agg_str = """
            {{~#if this.command=="FeatureTransAgg"}}
            "command_args":{
                "agg_args": {
                    {{~#geneach "this.agg_args" join=', ' stop='}' }}
...
feature_trans_clip_str = """
            {{~#if this.command=="FeatureTransClip"}}
            "command_args":{
...
#                 "bool_args": {{~gen "this.bool_args" temperature=0.01}}
#                 },
#             {{~/if}}"""
replace_str = """
            {{~#if this.command=="Replace"}}
...
rename_str = """
            ...
            "command_args": {
                ...
                "index_rename": {{~gen "this.index_rename" temperature=1.0 stop='}'}}
                },          
            {{~/if}}"""

# Sort
sort_str = """
            {{~#if this.command=="SortValues"}}
            "command_args":{
...
# Resample
resample_str = """
            {{~#if this.command=="Resample"}}
            "command_args":{
                "time_columns": [{{~gen "this.time_columns" temperature=0.01 stop=']'}}],
                "columns": [{{~gen "this.columns" temperature=0.01 stop=']'}}],
                "freq": "{{~gen "this.freq" temperature=0.01 stop='"'}}",
                "how": "{{~gen "this.how" temperature=0.01 stop='"'}}",
                "fill_method": "{{~gen "this.fill_method" temperature=0.01 stop='"'}}"
                },
            {{~/if}}"""

# TimeInterval_str
timeinterval_str = """
            {{~#if this.command=="TimeInterval"}}
            "command_args":{
                "columns": [{{~gen "this.columns" temperature=0.01 stop=']'}}],
